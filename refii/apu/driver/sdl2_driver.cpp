...
static void AudioThread()
{
    ...
    while (!g_audioThreadShouldExit)
    {
        ...
        if ((queuedAudioSize / callbackAudioSize) <= MAX_LATENCY)
        {
            ...
            g_clientCallback(ctx.ppcContext, refii::kernel::g_memory.base);
        }
...
{
    auto* pClientParam = static_cast<uint32_t*>(refii::kernel::g_userHeap.Alloc(sizeof(param)));
    ByteSwapInplace(param);
    *pClientParam = param;
    g_clientCallbackParam = refii::kernel::g_memory.MapVirtual(pClientParam);
