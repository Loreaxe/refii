...
static refii::kernel::Mutex g_copyMutex;
...
static refii::kernel::Mutex g_pipelineCacheMutex;
#endif
...
//
static refii::kernel::Mutex g_pipelineTaskMutex;
...
static refii::kernel::Mutex g_vertexDeclarationMutex;
    ...
    for (auto resource : g_tempResources[g_frame])
    {
        ...
        case ResourceType::VolumeTexture:
        {
            ...
            if (texture->mappedMemory != nullptr)
                refii::kernel::g_userHeap.Free(texture->mappedMemory);
        ...
        case ResourceType::IndexBuffer:
        {
            ...
            if (buffer->mappedMemory != nullptr)
                refii::kernel::g_userHeap.Free(buffer->mappedMemory);
        ...
        refii::kernel::g_userHeap.Free(resource);
    }
...
static void* LockBuffer(GuestBuffer* buffer, uint32_t flags)
{
    ...
    if (buffer->mappedMemory == nullptr)
        buffer->mappedMemory = refii::kernel::g_userHeap.AllocPhysical(buffer->dataSize, 0x10);
...
static void DrawProfiler()
{
    ...
    if (ImGui::Begin("Profiler", &g_profilerVisible))
    {
        ...
        if (refii::kernel::g_userHeap.heap != nullptr && refii::kernel::g_userHeap.physicalHeap != nullptr)
        {
            O1HeapDiagnostics diagnostics, physicalDiagnostics;
            {
                std::lock_guard lock(refii::kernel::g_userHeap.mutex);
                diagnostics = o1heapGetDiagnostics(refii::kernel::g_userHeap.heap);
            }
            {
                std::lock_guard lock(refii::kernel::g_userHeap.physicalMutex);
                physicalDiagnostics = o1heapGetDiagnostics(refii::kernel::g_userHeap.physicalHeap);
            }
...
static RenderShader* GetOrLinkShader(GuestShader* guestShader, uint32_t specConstants)
{
    ...
    if (shader == nullptr)
    {
        static refii::kernel::Mutex g_compiledSpecConstantLibraryBlobMutex;
...
static GuestVertexDeclaration* CreateVertexDeclarationWithoutAddRef(GuestVertexElement* vertexElements) 
{
    ...
    {
        vertexDeclaration = refii::kernel::g_userHeap.AllocPhysical<GuestVertexDeclaration>(ResourceType::VertexDeclaration);
...
static GuestShader* CreateShader(const be<uint32_t>* function, ResourceType resourceType)
{
    ...
    if (findResult != nullptr)
    {
        ...
        {
            shader = refii::kernel::g_userHeap.AllocPhysical<GuestShader>(resourceType);
...
uint32_t refii::gpu::CreateDevice(uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5, be<uint32_t>* a6)
{
    ...
    g_backBuffer = refii::kernel::g_userHeap.AllocPhysical<GuestSurface>(std::move(*g_backBufferHolder));
    ...
    auto device = refii::kernel::g_userHeap.AllocPhysical<GuestDevice>();
    memset(device, 0, sizeof(*device));

    // Append render state functions to the end of guest function table.
    uint32_t functionOffset = PPC_CODE_BASE + PPC_CODE_SIZE;
    refii::kernel::g_memory.InsertFunction(functionOffset, HostToGuestFunction<SetRenderStateUnimplemented>);
    ...
    {
        functionOffset += 4;
        refii::kernel::g_memory.InsertFunction(functionOffset, function);
...
static void refii::gpu::LockTextureRect(GuestTexture* texture, uint32_t, GuestLockedRect* lockedRect)
{
    uint32_t pitch = ComputeTexturePitch(texture);
    uint32_t slicePitch = pitch * texture->height;

    if (texture->mappedMemory == nullptr)
        texture->mappedMemory = refii::kernel::g_userHeap.AllocPhysical(slicePitch, 0x10);

    lockedRect->pitch = pitch;
    lockedRect->bits = refii::kernel::g_memory.MapVirtual(texture->mappedMemory);
}

static void refii::gpu::UnlockTextureRect(GuestTexture* texture)
{
...
static void* refii::gpu::LockVertexBuffer(GuestBuffer* buffer, uint32_t, uint32_t, uint32_t flags)
{
    return LockBuffer(buffer, flags);
}

static void refii::gpu::UnlockVertexBuffer(GuestBuffer* buffer)
{
    UnlockBuffer<uint32_t>(buffer);
}

static void refii::gpu::GetVertexBufferDesc(GuestBuffer* buffer, GuestBufferDesc* desc)
{
    desc->size = buffer->dataSize;
}

static void* refii::gpu::LockIndexBuffer(GuestBuffer* buffer, uint32_t, uint32_t, uint32_t flags)
{
    return LockBuffer(buffer, flags);
}

static void refii::gpu::UnlockIndexBuffer(GuestBuffer* buffer)
{
...
static void refii::gpu::GetIndexBufferDesc(GuestBuffer* buffer, GuestBufferDesc* desc)
{
    desc->format = buffer->guestFormat;
    desc->size = buffer->dataSize;
}

static void refii::gpu::GetSurfaceDesc(GuestSurface* surface, GuestSurfaceDesc* desc)
{
    desc->width = surface->width;
    desc->height = surface->height;
}

static void refii::gpu::GetVertexDeclaration(GuestVertexDeclaration* vertexDeclaration, GuestVertexElement* vertexElements, be<uint32_t>* count)
{
...
static uint32_t refii::gpu::HashVertexDeclaration(uint32_t vertexDeclaration)
{
    // Vertex declarations are cached on host side, so the pointer itself can be used.
    return vertexDeclaration;
}

static GuestSurface* refii::gpu::GetBackBuffer()
{
    g_backBuffer->AddRef();
    return g_backBuffer;
}

static GuestTexture* refii::gpu::CreateTexture(uint32_t width, uint32_t height, uint32_t depth, uint32_t levels, uint32_t usage, uint32_t format, uint32_t pool, uint32_t type)
{
    const auto texture = refii::kernel::g_userHeap.AllocPhysical<GuestTexture>(type == 17 ? ResourceType::VolumeTexture : ResourceType::Texture);
...
#ifdef _DEBUG 
    texture->texture->setName(fmt::format("Texture {:X}", refii::kernel::g_memory.MapVirtual(texture)));
#endif

    return texture;
}

static GuestBuffer* refii::gpu::CreateVertexBuffer(uint32_t length)
{
    auto buffer = refii::kernel::g_userHeap.AllocPhysical<GuestBuffer>(ResourceType::VertexBuffer);
    buffer->buffer = g_device->createBuffer(RenderBufferDesc::VertexBuffer(length, GetBufferHeapType(), RenderBufferFlag::INDEX));
    buffer->dataSize = length;
#ifdef _DEBUG 
    buffer->buffer->setName(fmt::format("Vertex Buffer {:X}", refii::kernel::g_memory.MapVirtual(buffer)));
#endif
    return buffer;
}

static GuestBuffer* refii::gpu::CreateIndexBuffer(uint32_t length, uint32_t, uint32_t format)
{
    auto buffer = refii::kernel::g_userHeap.AllocPhysical<GuestBuffer>(ResourceType::IndexBuffer);
...
#ifdef _DEBUG 
    buffer->buffer->setName(fmt::format("Index Buffer {:X}", refii::kernel::g_memory.MapVirtual(buffer)));
#endif
    return buffer;
}

static GuestSurface* refii::gpu::CreateSurface(uint32_t width, uint32_t height, uint32_t format, uint32_t multiSample)
{
    RenderTextureDesc desc;
    desc.dimension = RenderTextureDimension::TEXTURE_2D;
    desc.width = width;
    ...
    auto surface = refii::kernel::g_userHeap.AllocPhysical<GuestSurface>(desc.format == RenderFormat::D32_FLOAT ?
...
#ifdef _DEBUG 
    surface->texture->setName(fmt::format("{} {:X}", desc.flags & RenderTextureFlag::RENDER_TARGET ? "Render Target" : "Depth Stencil", refii::kernel::g_memory.MapVirtual(surface)));
#endif

    return surface;
}

static void refii::gpu::StretchRect(GuestDevice* device, uint32_t flags, uint32_t, GuestTexture* texture)
{
    RenderCommand cmd;
...
static void refii::gpu::SetRenderTarget(GuestDevice* device, uint32_t index, GuestSurface* renderTarget)
{
    if (renderTarget == nullptr)
    {
        return;
...
static void refii::gpu::SetDepthStencilSurface(GuestDevice* device, GuestSurface* depthStencil)
{
    RenderCommand cmd;
...
static void refii::gpu::Clear(GuestDevice* device, uint32_t count, be<uint32_t>* pRects, uint32_t flags, uint32_t, be<float>* color, double z, uint32_t stencil, bool EDRAMClear)
{
    RenderCommand cmd;
...
static void refii::gpu::SetViewport(GuestDevice* device, GuestViewport* viewport)
{
    RenderCommand cmd;
    cmd.type = RenderCommandType::SetViewport;
...
static void refii::gpu::SetTexture(GuestDevice* device, uint32_t index, GuestTexture* texture)
{
...
static void refii::gpu::SetScissorRect(GuestDevice* device, GuestRect* rect)
{
    RenderCommand cmd;
...
static void refii::gpu::DrawPrimitive(GuestDevice* device, uint32_t primitiveType, uint32_t startVertex, uint32_t primitiveCount)
{
...
static void refii::gpu::DrawIndexedPrimitive(GuestDevice* device, uint32_t primitiveType, int32_t baseVertexIndex, uint32_t startIndex, uint32_t primCount)
{
    LocalRenderCommandQueue queue;
    FlushRenderStateForMainThread(device, queue);
...
static void refii::gpu::DrawPrimitiveUP(GuestDevice* device, uint32_t primitiveType, uint32_t primitiveCount, void* vertexStreamZeroData, uint32_t vertexStreamZeroStride)
{
    LocalRenderCommandQueue queue;
    FlushRenderStateForMainThread(device, queue);
...
static GuestVertexDeclaration* refii::gpu::CreateVertexDeclaration(GuestVertexElement* vertexElements)
{
...
static void refii::gpu::SetVertexDeclaration(GuestDevice* device, GuestVertexDeclaration* vertexDeclaration)
{
    RenderCommand cmd;
    ...
    device->vertexDeclaration = refii::kernel::g_memory.MapVirtual(vertexDeclaration);
}

static GuestShader* refii::gpu::CreateVertexShader(const be<uint32_t>* function)
{
    return CreateShader(function, ResourceType::VertexShader);
}

static void refii::gpu::SetVertexShader(GuestDevice* device, GuestShader* shader)
{
    RenderCommand cmd;
...
static void refii::gpu::SetStreamSource(GuestDevice* device, uint32_t index, GuestBuffer* buffer, uint32_t offset, uint32_t stride)
{
    RenderCommand cmd;
...
static void refii::gpu::SetIndices(GuestDevice* device, GuestBuffer* buffer)
{
    RenderCommand cmd;
...
static GuestShader* refii::gpu::CreatePixelShader(const be<uint32_t>* function)
{
    return CreateShader(function, ResourceType::PixelShader);
}

static void refii::gpu::SetPixelShader(GuestDevice* device, GuestShader* shader)
{
    RenderCommand cmd;
...
static void refii::gpu::D3DXFillTexture(GuestTexture* texture, uint32_t function, void* data)
{
...
static void refii::gpu::D3DXFillVolumeTexture(GuestTexture* texture, uint32_t function, void* data)
{
...
static void refii::gpu::MakePictureData(GuestPictureData* pictureData, uint8_t* data, uint32_t dataSize)
{
    if ((pictureData->flags & 0x1) == 0 && data != nullptr)
    {
        ...
        {
#ifdef _DEBUG
            texture.texture->setName(reinterpret_cast<char*>(refii::kernel::g_memory.Translate(pictureData->name + 2)));
#endif
            ...
            pictureData->texture = refii::kernel::g_memory.MapVirtual(refii::kernel::g_userHeap.AllocPhysical<GuestTexture>(std::move(texture)));
            pictureData->type = 0;
        }
    }
}

static void refii::gpu::SetResolution(be<uint32_t>* device)
{
    Video::ComputeViewportDimensions();
...
static GuestShader* refii::gpu::CreateMoviePixelShader(be<uint32_t>* hlslShader)
{
    if (g_moviePixelShader == nullptr)
    {
        g_moviePixelShader = refii::kernel::g_userHeap.AllocPhysical<GuestShader>(ResourceType::PixelShader);
...
static GuestShader* refii::gpu::CreateMovieVertexShader(be<uint32_t>* hlslShader)
{
    if (g_movieVertexShader == nullptr)
    {
        g_movieVertexShader = refii::kernel::g_userHeap.AllocPhysical<GuestShader>(ResourceType::VertexShader);
